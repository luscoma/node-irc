    ...
    self.addListener("raw", function (message) { // {{{
        switch ( message.command ) {
            ...
            case "PRIVMSG":
                ...
                if ( text[0] === '\1' && text[text.length-1] === '\1') self.emit('ctcp', from, text.slice(1,text.length-1));
...
Client.prototype.action = function(target, text) { // {{{
    this.send('PRIVMSG', target, "\1ACTION " + text + "\1");            // actions are of the form ACTION target :\1ACTION text\1
} // }}}
Client.prototype.ctcp = function(target, text) { // {{{
    this.send('PRIVMSG', target, "\1" + text + "\1");                   // ctcp commands are of the form PRIVMSG target :\1CMD\1
} // }}}
Client.prototype.ctcpReply = function(target, ctcp, text) { // {{{
    this.send('NOTICE', target, "\1" + ctcp + " " + text + "\1");    // ctcp replys are of the form NOTICE target :\1CMD Reply\1
} // }}}
